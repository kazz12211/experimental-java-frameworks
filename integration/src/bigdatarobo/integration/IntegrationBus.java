package bigdatarobo.integration;
...
import bigdatarobo.integration.bus.BusListener;
import bigdatarobo.integration.bus.InboundListener;
import bigdatarobo.integration.bus.OutboundListener;
import bigdatarobo.integration.channel.Channel;
import bigdatarobo.integration.channel.InputChannel;
import bigdatarobo.integration.channel.OutputChannel;
import bigdatarobo.integration.message.BusMessage;
import bigdatarobo.integration.message.MessageType;
import bigdatarobo.integration.util.Log;
...
public class IntegrationBus implements Runnable {
	...
	public void start() {
		...
		Log.integration.debug("IntegrationBus creating " + nThreads + " worker threads");
		...
		Log.integration.debug("IntegrationBus started");
	}
	...
	public void destroy() {
		...
		Log.integration.debug("IntegrationBus stopped");
	}
	...
	public void post(BusMessage message) throws Exception {
		...
		Log.integration.debug("IntegrationBus queueing message " + message);
		synchronized(messageQueue) {
	...
	private void sendFailure(Channel channel, BusMessage message, Throwable error) {
		Log.integration.debug("IntegrationBus sendFailure(" + channel + ", " + message.getType() + ")");
	...
	private void willSend(Channel channel, BusMessage message) {
		Log.integration.debug("IntegrationBus willSend(" + channel + ", " + message.getType() + ")");
	...
	private void complete(Channel channel, BusMessage message) {
		Log.integration.debug("IntegrationBus complete(" + channel + ", " + message.getType() + ")");
	...
	private void onMessage(Channel channel, BusMessage message) {
		Log.integration.debug("IntegrationBus onMessage(" + channel + ", " + message.getType() + ")");
